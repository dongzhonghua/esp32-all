...
const char *app_event_type_info[] = {
    "APP_MESSAGE_WIFI_CONN",   "APP_MESSAGE_WIFI_AP",
...
AppController::AppController(const char *name) {
  ...
  // 定义一个事件处理定时器
  xTimerEventDeal = xTimerCreate("Event Deal", 300 / portTICK_PERIOD_MS, pdTRUE,
                                 (void *)0, eventDealHandle);
  // 启动事件处理定时器
...
int AppController::app_is_legal(const APP_OBJ *app_obj) {
  // APP的合法性检测
  if (NULL == app_obj) return 1;
  if (APP_MAX_NUM <= app_num) return 2;
  return 0;
}
...
int AppController::app_install(APP_OBJ *app, APP_TYPE app_type) {
  ...
  appList[app_num] = app;
  Serial.printf("app: %s, index: %d \n", app->app_name, app_num);
...
int AppController::main_process(ImuAction *act_info) {
  ...
  // 1.如果当前没有运行app，则根据active_type执行左右切换或者进入APP或者什么都不做。
  // 2.如果有运行的app，则执行相应app的main_process，作用是由相应的app来控制屏幕。
  // 退出APP后，则相应的又回到这里的 1。
  if (0 == app_exit_flag) {
    // 当前没有进入任何app
    ...
    if (ACTIVE_TYPE::TURN_LEFT == act_info->active) {
      ...
      Serial.println(String("Current App: ") +
                     appList[cur_app_index]->app_name);
      ...
      cur_app_index = (cur_app_index - 1 + app_num) %
                      app_num;  // 此处的3与p_processList的长度一致
      Serial.println(String("Current App: ") +
                     appList[cur_app_index]->app_name);
    ...
    if (ACTIVE_TYPE::GO_FORWORD !=
        act_info->active)  // && UNKNOWN != act_info->active
    {
      app_control_display_scr(appList[cur_app_index]->app_image,
                              appList[cur_app_index]->app_name, anim_type,
                              false);
      delay(300);
    }
  } else {
...
int AppController::send_to(const char *from, const char *to,
  ...
  if (type <= APP_MESSAGE_MQTT_DATA) {
    ...
    // 各个APP之间通信的消息
    if (NULL != toApp) {
      Serial.print("[Massage]\tFrom " + String(fromApp->app_name) + "\tTo " +
                   String(toApp->app_name) + "\n");
...
int AppController::req_event_deal(void) {
  // 请求事件的处理
  for (std::list<EVENT_OBJ>::iterator event = eventList.begin();
       event != eventList.end();) {
    ...
    if (false == ret) {
      ...
      if ((*event).retryCount >= (*event).retryMaxNum) {
        // 多次重试失败
        Serial.print("[EVENT]\tDelete -> " +
                     String(app_event_type_info[(*event).type]));
...
bool AppController::wifi_event(APP_MESSAGE_TYPE type) {
  ...
  //         g_network.start_conn_wifi(sys_cfg.ssid_0.c_str(),
  //                                   sys_cfg.password_0.c_str());
  //         m_wifi_status = true;
  //       }
  //       m_preWifiReqMillis = millis();
  //       if ((WiFi.getMode() & WIFI_MODE_STA) == WIFI_MODE_STA &&
  //           CONN_SUCC != g_network.end_conn_wifi()) {
  //         // 在STA模式下 并且还没连接上wifi
  //         return false;
  //       }
  //     } break;
  ...
  //       if (app_exit_flag == 1 &&
  //           cur_app_index != getAppIdxByName("Heartbeat"))  // 在其他app中
  //       {
  //         app_exit_flag = 0;
