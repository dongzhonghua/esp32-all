...
// 天气图标路径的映射关系
const void *weaImage_map[] = {&weather_0,  &weather_9,  &weather_14,
                              &weather_5,  &weather_25, &weather_30,
                              &weather_26, &weather_11, &weather_23};
// 太空人图标路径的映射关系
const void *manImage_map[] = {&man_0, &man_1, &man_2, &man_3, &man_4,
                              &man_5, &man_6, &man_7, &man_8, &man_9};
static const char weekDayCh[7][4] = {"日", "一", "二", "三", "四", "五", "六"};
static const char airQualityCh[6][10] = {"优",   "良",   "轻度",
                                         "中度", "重度", "严重"};
...
void display_curve_init(lv_scr_load_anim_t anim_type) {
  ...
  ser2 = lv_chart_add_series(chart, lv_palette_main(LV_PALETTE_BLUE),
                             LV_CHART_AXIS_SECONDARY_Y);
  // lv_obj_set_style_pad_left(chart, LV_CHART_PART_BG, LV_STATE_DEFAULT, 40);
  // lv_chart_set_y_tick_texts(chart, "40\n30\n20\n10\n0", 0,
  // LV_CHART_AXIS_DRAW_LAST_TICK); lv_chart_set_zoom_y(); 绘制
...
void display_curve(short maxT[], short minT[], lv_scr_load_anim_t anim_type) {
  ...
  lv_chart_set_range(chart, LV_CHART_AXIS_PRIMARY_Y, min_num(minT, 7), max_num(maxT, 7));
  lv_chart_set_range(chart, LV_CHART_AXIS_SECONDARY_Y, min_num(minT, 7), max_num(maxT, 7));
	lv_chart_set_axis_tick(chart, LV_CHART_AXIS_PRIMARY_X, 10, 5, 7, 1, true, 50);
	lv_chart_set_axis_tick(chart, LV_CHART_AXIS_PRIMARY_Y, 10, 5, 7, 2, true, 50);
  lv_chart_refresh(chart);
}

void display_weather_init(lv_scr_load_anim_t anim_type) {
  lv_obj_t *act_obj = lv_scr_act();  // 获取当前活动页
  if (act_obj == scr_1) return;
  ...
  lv_obj_set_size(txtLabel, 140, 30);
  lv_label_set_long_mode(txtLabel, LV_LABEL_LONG_SCROLL_CIRCULAR);
  lv_label_set_text_fmt(txtLabel, "最低气温%d°C, 最高气温%d°C, %s%d 级.   ", 15,
                        20, "西北风", 0);
  ...
  lv_obj_set_style_bg_color(tempBar, lv_palette_main(LV_PALETTE_RED),
                            LV_PART_MAIN);
  ...
  lv_obj_set_style_bg_color(humiBar, lv_palette_main(LV_PALETTE_BLUE),
                            LV_PART_MAIN);
...
void display_weather(struct Weather weaInfo, lv_scr_load_anim_t anim_type) {
  ...
  lv_label_set_text_fmt(txtLabel, "最低气温%d°C, 最高气温%d°C, %s%d 级.   ",
                        weaInfo.minTemp, weaInfo.maxTemp, weaInfo.windDir,
                        weaInfo.windLevel);
...
void display_time(struct TimeStr timeInfo, lv_scr_load_anim_t anim_type) {
  display_weather_init(anim_type);
  lv_label_set_text_fmt(clockLabel_1, "%02d#ffa500 %02d#", timeInfo.hour,
                        timeInfo.minute);
  lv_label_set_text_fmt(clockLabel_2, "%02d", timeInfo.second);
  lv_label_set_text_fmt(dateLabel, "%2d月%2d日   周%s", timeInfo.month,
                        timeInfo.day, weekDayCh[timeInfo.weekday]);
...
short max_num(short numbers[], int size) {
  int count;
  short highest;
  highest = numbers[0];
...
short min_num(short numbers[], int size) {
  int count;
  short lowest;
  lowest = numbers[0];
  for (count = 1; count < size; count++) {
