...
#define WEATHER_NOW_API              \
  "https://www.yiketianqi.com/free/" \
  "day?appid=%s&appsecret=%s&unescape=1&city=%s"

#define WEATHER_NOW_API_UPDATE \
  "https://yiketianqi.com/"    \
  "api?unescape=1&version=v6&appid=%s&appsecret=%s&city=%s"

#define WEATHER_DALIY_API            \
  "https://www.yiketianqi.com/free/" \
  "week?unescape=1&appid=%s&appsecret=%s&city=%s"
...
static void read_config(WT_Config *cfg) {
  ...
  if (size == 0) {
    ...
    cfg->tianqi_appid = "25683162";
    cfg->tianqi_appsecret = "9cZLsSWC";
...
std::map<String, int> weatherMap = {{"qing", 0},    {"yin", 1},     {"yu", 2},
                                    {"yun", 3},     {"bingbao", 4}, {"wu", 5},
                                    {"shachen", 6}, {"lei", 7},     {"xue", 8}};
...
static void get_weather(void) {
  ...
  char api[128] = {0};
  // snprintf(api, 128, WEATHER_NOW_API, cfg_data.tianqi_appid,
  // cfg_data.tianqi_appsecret, cfg_data.tianqi_addr);
  snprintf(api, 128, WEATHER_NOW_API_UPDATE, cfg_data.tianqi_appid.c_str(),
           cfg_data.tianqi_appsecret.c_str(), cfg_data.tianqi_addr.c_str());
  Serial.print("API = ");
  Serial.println(api);
  http.begin(api);
...
static long long get_timestamp(String url) {
  ...
  if (httpCode > 0) {
    if (httpCode == HTTP_CODE_OK) {
      ...
      // 以网络时间戳为准
      run_data->preNetTimestamp =
          atoll(time.c_str()) + run_data->errorNetTimestamp + TIMEZERO_OFFSIZE;
      run_data->preLocalTimestamp = millis();
    }
  } else {
    Serial.printf("[HTTP] GET... failed, error: %s\n",
...
static void get_daliyWeather(short maxT[], short minT[]) {
  ...
  http.setTimeout(1000);
  char api[128] = {0};
  snprintf(api, 128, WEATHER_DALIY_API, cfg_data.tianqi_appid.c_str(),
           cfg_data.tianqi_appsecret.c_str(), cfg_data.tianqi_addr.c_str());
  Serial.print("API = ");
  Serial.println(api);
  http.begin(api);
...
static int weather_init(AppController *sys) {
  ...
  run_data->preNetTimestamp =
      1577808000000;  // 上一次的网络时间戳 初始化为2020-01-01 00:00:00
...
static void weather_process(AppController *sys, const ImuAction *act_info) {
  ...
  if (RETURN == act_info->active) {
    ...
    // 以下等效与 clock_page = (clock_page + WEATHER_PAGE_SIZE - 1) %
    // WEATHER_PAGE_SIZE; +3为了不让数据溢出成负数，而导致取模逻辑错误
    run_data->clock_page =
  ...
  if (run_data->clock_page == 0) {
    ...
    if (0x01 == run_data->coactusUpdateFlag ||
        doDelayMillisTime(cfg_data.weatherUpdataInterval,
                          &run_data->preWeatherMillis, false)) {
      sys->send_to(WEATHER_APP_NAME, CTRL_NAME, APP_MESSAGE_WIFI_CONN,
                   (void *)UPDATE_NOW, NULL);
      sys->send_to(WEATHER_APP_NAME, CTRL_NAME, APP_MESSAGE_WIFI_CONN,
                   (void *)UPDATE_DAILY, NULL);
    }

    if (0x01 == run_data->coactusUpdateFlag ||
        doDelayMillisTime(cfg_data.timeUpdataInterval, &run_data->preTimeMillis,
                          false)) {
      // 尝试同步网络上的时钟
      sys->send_to(WEATHER_APP_NAME, CTRL_NAME, APP_MESSAGE_WIFI_CONN,
                   (void *)UPDATE_NTP, NULL);
